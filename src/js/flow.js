import { addClass, removeClass, raf, random } from 'src/js/utils';
...
[...document.querySelectorAll('.link')].forEach((link) => {
  link.addEventListener('mouseenter', hover, { passive: true });
  link.addEventListener('mouseleave', leave, { passive: true });
});
...
function getColor(rotate, alpha) {
  const rgbFloor = 148;
...
function updatePoints({
  ...
  const add = flowFast ? delta : delta * 0.4;
